...
def get_dir_size(p_ath):
    total_size = 0
    for entry in os.scandir(p_ath):
        if entry.is_dir():
            total_size += get_dir_size(entry.path)  # заново запускаем директорию в функцию
        else:
...
def dir_list_of_dict(dir_path):
    ...
    for dirpath, dirnames, filenames in os.walk(dir_path):
        for dir in dirnames:
            ...
            dir_dict.update({'size': get_dir_size(os.path.join(dirpath, dir))})  # размер папки
